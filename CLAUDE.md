...
Dice trigger **left to right in the order they physically sit in the pit** when the player takes the hand: `scene_root._on_take_button_pressed` sorts the selection by each die body's Z and passes it to `ScoreBreakdown.build(…, eye_order)`, so the counting sweep matches the row the player already sees (`_line_up_settled_dice`: selected-first, value-desc, slot-asc). This is safe **only because every per-die step is additive** (eye/material/per-die-charm all `+=`; factors live in the charm phase) — the order changes the display, never the total. `participating_indices` still returns sorted indices (its default, and what pure-logic tests assert); Echo-Kammer's target stays the min index via `CharmEffects.first_participating`. **If you ever add a per-die *factor* (multiplicative) hook, physical order would start changing the score — don't, or the take result becomes arrangement-dependent.** A charm effect that concerns a single participating die fires **inside that die's step** via the per-die hooks `CharmEffects.die_charm_base_at`/`die_charm_mult_at` (Breitband, Straßenkehrer, Zargenglanz, Nachzügler, Bodensatz). After all dice, charms fire **strictly in ownership (dock) order** — additive bonuses AND factors (Einserkult, Feierabendbier, Galgenhumor) apply at their exact position via the `charm_*_at(j, …)` hooks, never pooled at the end, so dock order is gameplay-relevant. **Crits** are factors on the *current* mult (mult 10, crit ×3 → 30), applied at the owner's position like every factor — hook `CharmEffects.charm_crit_at` (Galgenhumor ×4); `ScoreBreakdown` exposes them as `crit_x` on the charm step so the UI can stage them. After the base × mult merge, total-level effects (`charm_total_add_at`/`charm_total_factor_at` — Regenbogenforelle, Zauberkarte) also run in ownership order. `ScoreBreakdown.build` mirrors this order 1:1 for the counting animation; when adding a per-die charm, put it in the die hooks — **not** in `charm_base_bonus_at`/`charm_mult_bonus_at` — or the animation will fire it in the wrong phase.

**Ties on a single-die charm (fixed rule).** When a charm means exactly *one* die picked by its face value (Hochstapler/Leuchtturm = highest, Beherit = lowest, Flickenteppich = highest), a tie is broken by **the first matching die — the smallest slot**. Never scan for the extreme value yourself; call `CharmEffects.target_die(values, candidates, highest)`, the single source of truth (it walks ascending and only takes a *strictly* better value, so the first wins). `ScoreBreakdown` publishes the chosen die as `slot` on the charm step (`-1` = none) and, for additive charms, as the step's per-die pulse, so the animation flashes the die that actually triggered — the crit path does the same via `_play_crit_step`.
