...
- `old_penny.glb` **Glücksgroschen** — An antique copper penny lovingly polished to a warm shine, crisp embossing, resting on a plump red velvet cushion.
...
- ✔ `double_bottom.glb` **Doppelter Boden** — An elegant polished briefcase with a lifted false-bottom panel revealing neatly stacked gold bars.
...
- `emergency_fund.glb` **Notgroschen** — A glossy red wall box with a crystal-clear break-glass front, one golden coin inside, a polished little hammer on a neat chain.
...
- `display_case.glb` **Vitrine** — A spotless glass display case with two lit shelves and empty velvet pedestals, slim polished gold frame.
...
- `hermit_crab.glb` **Einsiedlerkrebs** — A cute hermit crab wearing a neat spiral stack of glossy casino chips as its shell, polished chrome legs, antenna tips glowing.
...
- `swamp_lantern.glb` **Sumpflaterne** — An elegant lantern with an eerie green flame behind spotless glass, delicate decorative vines curling up its polished frame.
...
- `solar_eclipse.glb` **Sonnenfinsternis** — A gleaming brass orrery frozen mid-eclipse: a polished black disc sliding before a glowing golden orb, a crisp corona ring visible.
...
- `printing_press.glb` **Druckerpresse** — A gleaming tabletop hand press with a polished lever, crisp freshly printed glowing pattern sheets stacked beside it.
...
- `fenced_goods.glb` **Hehlerware** — A neat wooden crate with the lid levered open, glossy luxury goods nestled in clean straw, a polished crowbar resting against it.
...
- `pressure_gauge.glb` **Manometer** — A gleaming brass pressure gauge with a spotless white dial, its red needle poised near the top of the scale.
- `empty_plinth.glb` **Leerer Sockel** — An immaculate white marble plinth with a velvet rope barrier, empty under a crisp spotlight, a faint outline where the exhibit stood.
