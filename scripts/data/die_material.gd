...
static func ruby() -> DieMaterial:
	return _make(RUBY, "Rubin", "+4 Mult, wenn diese Seite in der Kombination liegt.", Color(0.94, 0.45, 0.5))

## Bernstein: +20 Augen beim Zählen.
static func amber() -> DieMaterial:
	return _make(AMBER, "Bernstein", "+20 Augen beim Zählen, wenn diese Seite in der Kombination liegt.", Color(1.0, 0.78, 0.42))

## Gold: +$1 beim Nehmen.
static func gold() -> DieMaterial:
	return _make(GOLD, "Gold", "+$1, wenn diese Seite in der genommenen Kombination liegt.", Color(1.0, 0.88, 0.45))

## Knochen: die Seite wächst beim Nehmen dauerhaft um +1.
static func bone() -> DieMaterial:
	return _make(BONE, "Knochen", "Diese Seite wächst dauerhaft +1, wenn sie in der genommenen Kombination liegt.", Color(0.93, 0.9, 0.78))

## Quecksilber: die Seite zählt doppelt.
static func mercury() -> DieMaterial:
	return _make(MERCURY, "Quecksilber", "Diese Seite zählt doppelt, wenn sie in der Kombination liegt.", Color(0.78, 0.83, 0.92))

## Glas: Mult += Augen der Seite, danach schrumpft sie dauerhaft um −1.
static func glass() -> DieMaterial:
	return _make(GLASS, "Glas", "Mult += Augen dieser Seite; beim Nehmen schrumpft sie dauerhaft −1 (min. 1).", Color(0.68, 0.88, 0.95))
