...
func test_a_copper_face_feeds_one_energy_per_firing():
	var report := _take(_copper_die())
	assert_eq(report.copper_energy, MaterialEffects.COPPER_ENERGY)
	assert_eq(report.copper, [0] as Array[int], "der Slot meldet seine Zündung")

func test_a_doped_copper_face_feeds_twice():
	assert_eq(_take(_copper_die(true)).copper_energy, MaterialEffects.COPPER_ENERGY_DOPED)
...
func test_copper_fires_once_per_activation_on_both_axes():
	...
	assert_eq(report.copper_energy, 4, "2 Würfel-Trigger × 2 Seiten-Zündungen")
	assert_eq(report.copper.size(), 4, "je Zündung ein Eintrag")

func test_the_kiln_doubles_the_doped_payout():
	assert_eq(_take(_copper_die(true), _ids([Charm.KILN])).copper_energy,
		MaterialEffects.COPPER_ENERGY_DOPED * 2)

func test_the_kiln_leaves_the_undoped_face_alone():
	assert_eq(_take(_copper_die(), _ids([Charm.KILN])).copper_energy, MaterialEffects.COPPER_ENERGY)
...
func test_another_material_feeds_nothing():
	...
	assert_eq(report.copper_energy, 0)
	assert_true(report.copper.is_empty())

func test_copper_stays_out_of_the_spark_channel():
	# report.energy gehört dem Funkenflug: nur er läuft OHNE Geld-Überlauf.
	assert_eq(_take(_copper_die()).energy, 0)

func test_the_energy_of_one_firing_is_one_source():
	assert_eq(MaterialEffects.copper_energy_once_for(DieMaterial.COPPER, 1, NO_CHARMS), 1)
	assert_eq(MaterialEffects.copper_energy_once_for(DieMaterial.COPPER, DieMaterial.MAX_LEVEL, NO_CHARMS), 2)
	assert_eq(MaterialEffects.copper_energy_once_for(DieMaterial.GOLD, 1, NO_CHARMS), 0)

# --- Buchung: Überlauf zahlt bar ------------------------------------------------

func test_the_energy_lands_in_the_wallet():
	var run := GameRun.new_run()
	assert_eq(run.book_copper_energy(3), 0, "nichts läuft über")
	assert_eq(run.energy, 3)
	assert_eq(run.money, 0)

func test_a_full_wallet_pays_the_overflow_in_cash():
	var run := GameRun.new_run()
	run.energy = run.energy_cap() - 1
	var paid := run.book_copper_energy(4)
	assert_eq(run.energy, run.energy_cap(), "die Börse füllt sich bis zum Deckel")
...
func test_a_completely_full_wallet_pays_everything_in_cash():
	...
	assert_eq(run.book_copper_energy(2), 2 * MaterialEffects.COPPER_OVERFLOW_MONEY)
	assert_eq(run.energy, run.energy_cap(), "der Deckel bleibt der Deckel")

func test_booking_nothing_does_nothing():
	var run := GameRun.new_run()
	assert_eq(run.book_copper_energy(0), 0)
