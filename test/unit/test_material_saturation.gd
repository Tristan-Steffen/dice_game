...
func test_level_one_and_zero_are_identity() -> void:
	...
	for probe in probes:
		assert_eq(DieMaterial.saturated(probe, 0), probe, "ohne Material bleibt die Farbe")
		assert_eq(DieMaterial.saturated(probe, 1), probe, "normal ist der Regelfall")
...
func test_the_default_keeps_every_old_caller_on_the_old_colour() -> void:
	for material in DieMaterial.all():
		assert_eq(DieMaterial.tint_for(material.id), material.tint,
			"%s ohne Angabe = die Farbe von vorher" % material.id)
		assert_eq(DieMaterial.tint_for(material.id, 1), material.tint)

func test_every_material_gets_richer_when_doped() -> void:
...
func test_saturation_and_value_stay_in_range() -> void:
	for material in DieMaterial.all():
		for level in [1, DieMaterial.MAX_LEVEL]:
			...
			assert_between(color.s, 0.0, 1.0, "%s Zustand %d: S im Rahmen" % [material.id, level])
			assert_between(color.v, 0.0, 1.0, "%s Zustand %d: V im Rahmen" % [material.id, level])
