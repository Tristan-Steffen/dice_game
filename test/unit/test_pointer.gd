...
func test_a_link_crit_fires_at_the_dies_position():
	...
	var linked := DiceScoring.score_category(DiceScoring.TWO_KIND, dice,
		_ids([]), false, mats, {}, _ctx_fires(0, [_link(2, 3, DieMaterial.RUBY, DieMaterial.MAX_LEVEL)]))
	assert_eq(linked, 207, "Basis 23 × Mult (2 ×2 + 5)")
...
func test_breakdown_carries_links_and_matches_the_score():
	...
	assert_almost_eq(float(links[0]["crit_x"]), 1.0, 0.0001, "Beherit kritet kein Glied mehr")
	var charm_steps: Array = breakdown["charm_steps"]
	assert_eq(charm_steps.size(), 1, "sein Schlag steht in der Charm-Phase")
	assert_almost_eq(float(charm_steps[0]["crit_x"]), 6.0, 0.0001, "1 + niedrigste gewertete 5")
